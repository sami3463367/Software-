...
func _build_plots() -> void:
	var wood := WB.mat(WB.WOOD, 0.85)
	var wood_dark := WB.mat(WB.WOOD_DARK, 0.85)
	var dirt := WB.mat(WB.DIRT, 1.0)
	var rng := RandomNumberGenerator.new()
	rng.seed = 42
	for plot in PLOTS:
		var z: float = -1.0 + plot * PLOT_GAP
		# dirt bed
		var dirt_mesh := WB.box(
		...
		for cell in CELLS_PER_PLOT:
			var row := WB.box(Vector3(CELL_GAP - 0.15, 0.03, 1.1),
				WB.mat(Color(0.38, 0.26, 0.15), 1.0))
