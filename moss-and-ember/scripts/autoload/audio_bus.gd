...
func _ready() -> void:
	...
	var stream := load("res://audio/music_main.tres")
	if stream:
...
func _process(delta: float) -> void:
	# Re-trigger music if the browser suspended the audio context before the
	# first user gesture (common on mobile). Retried once a second rather than
	# every frame so a blocked context does not spam the console.
	_retry_t += delta
	if _retry_t < 1.0:
		return
	_retry_t = 0.0
...
func play_sfx(which: String, volume_db: float = 0.0) -> void:
	if muted:
		return
	var path := "res://audio/sfx_%s.tres" % which
